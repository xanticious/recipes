...
| `npm run classify-ha` | Classify recipe HA from ingredients (`--catalog`, `--write`) |
...
- **State:** all app state lives in xState machines / actors. Do not use `useState()` (or other React state hooks) for app state. React is for rendering and displaying actor snapshots and for sending events. Navigation, the nav overlay, recipe focus mode, filters, theme, font size, keep-screen-on, Ingredients filters and details, Restaurants city filter and details, Meal Ideas (including prep time), and Random belong in machines.
